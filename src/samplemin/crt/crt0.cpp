...
{
  wchar_t** argvW = CommandLineToArgvW(GetCommandLineW(), &g_argc);
  ...
  for (int i = 0; i < g_argc; i++)
  {
    ...
    g_argv[i] = (char*)malloc(argv_size);
    wcstombs(g_argv[i], argvW[i], argv_size);
  }
  ...
  )
{
  ...
  //
  create_argc_and_argv();
  int exit_code = main(g_argc, g_argv);
  destroy_argc_and_argv();
...
{
  [[noreturn]] void panic()
  { __debugbreak(); *reinterpret_cast<void**>(nullptr) = nullptr; }
}
...
#if !(MINI_CRT0_IGNORED & MINI_CRT0_EXCEPTIONS)
...
#pragma region COM
#if !(MINI_CRT0_IGNORED & MINI_CRT0_COM)

void __declspec(noreturn) __stdcall _com_raise_error(HRESULT hr, IErrorInfo* perrinfo = nullptr)
{ detail::panic(); }
